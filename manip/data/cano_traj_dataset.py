...
class CanoObjectTrajDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # The released processed_data may include the window pickle but not the per-window object BPS cache files.
        # The provided demo scripts require `--use_random_frame_bps`, so we compute the cache on-demand if missing.
        if not os.path.exists(obj_bps_npy_path):
            ...
            # Atomic write to avoid partially-written files if multiple workers race.
